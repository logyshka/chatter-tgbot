...
class Database:
    ...
    db_path = __file__.replace("main.py", "db.db")

    def __init__(self, table: str, _db_path=None):
        self._table = table
        self._db_path = _db_path if _db_path else self.db_path
    ...
    def insert(self, insert_pairs: dict):
        ...
        connection = connect(self._db_path, check_same_thread=False)
    ...
    def set(self,set_pair:list, search_pair: list):
        connection = connect(self._db_path, check_same_thread=False)
        cursor = connection.cursor()
